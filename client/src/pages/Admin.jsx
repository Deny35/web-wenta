...
function SuccessMsg({ msg, onHide }) {
  ...
  if (!msg) return null;
  return <div className="mb-4 px-3 py-2 bg-green-50 border border-green-200 text-green-700 rounded text-sm">{msg}</div>;
}
  ...
  async function saveTile(e) {
    ...
    try {
      if (editingId) {
        ...
        await api.projects.add({ title, category, year, img: coverData, images: [], opis: '', short_desc: '', featured: false });
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-sm text-slate-600">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex border-b-2 border-slate-200 mb-6 gap-0">
          {[['projects','Projekty'], ['clients','Klienci']].map(([t, label]) => (
            <button
              key={t}
              onClick={() => setMainTab(t)}
              className={`px-5 py-2.5 text-sm font-bold border-b-2 -mb-0.5 transition-colors ${
                mainTab === t ? 'text-brand border-brand' : 'text-slate-400 border-transparent hover:text-slate-600'
              }`}
            >
              {label}
            </button>
