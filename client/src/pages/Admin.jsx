...
function readFileAsDataURL(file, maxW = 1200, quality = 0.75) {
...
function ServicesAdminSection({ showSuccess }) {
  ...
  function reset() {
    setEditingId(null); setTab('basic');
    setTitle(''); setShortDesc(''); setOpis(''); setTags('');
  ...
  function startEdit(s) {
    setEditingId(s.id); setTab('basic');
    setTitle(s.title);
    setShortDesc(s.short_desc || '');
    setOpis(s.opis || '');
    setTags(Array.isArray(s.tags) ? s.tags.join(', ') : '');
    setCoverData(s.img || '');
    setGalleryData(Array.isArray(s.images) ? s.images.slice() : []);
  }
  ...
  async function saveBasic(e) {
    ...
    try {
      ...
      if (editingId) {
        const changes = { title, short_desc: shortDesc, opis, tags: tagsArr };
        if (coverData && !coverData.startsWith('http')) changes.img = coverData;
        else if (coverData) changes.img = coverData;
        await api.services.update(editingId, changes);
        showSuccess('Usługa zaktualizowana!');
      } else {
        await api.services.add({ title, short_desc: shortDesc, opis, tags: tagsArr, img: coverData, images: [] });
        showSuccess('Usługa dodana!');
        reset();
      }
      await refresh();
    } catch (err) {
  ...
  async function saveGallery(e) {
    ...
    try {
      ...
      const images  = galleryData.length ? galleryData : (current.images || []);
      await api.services.update(editingId, { images });
      showSuccess('Zdjęcia zaktualizowane!');
      await refresh();
    } catch (err) {
      alert('Błąd zapisu: ' + err.message);
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-6 items-start">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        {(!isEditing || tab === 'basic') && (
          <form onSubmit={saveBasic}>
            <h3 className="text-base font-bold text-slate-800 mb-4">
              {isEditing ? 'Edytuj usługę' : 'Dodaj usługę'}
            </h3>
            ...
            <div className="mb-3">
              ...
              <input
                required value={title} onChange={e => setTitle(e.target.value)}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:outline-none focus:border-brand"
                placeholder="np. Projektowanie 3D"
              />
            </div>

            <div className="mb-3">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Krótki opis (kafelek)</label>
              <input
                value={shortDesc} onChange={e => setShortDesc(e.target.value)}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:outline-none focus:border-brand"
                placeholder="Widoczny na siatce usług na stronie głównej…"
              />
            </div>

            <div className="mb-3">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Długi opis (strona usług)</label>
              <textarea
                value={opis} onChange={e => setOpis(e.target.value)} rows={5}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:outline-none focus:border-brand resize-vertical"
                placeholder="Pełny opis na stronie /uslugi…"
              />
            </div>

            <div className="mb-3">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Tagi (oddzielone przecinkiem)</label>
              <input
                value={tags} onChange={e => setTags(e.target.value)}
                className="w-full px-3 py-2 border border-slate-200 rounded text-sm focus:outline-none focus:border-brand"
                placeholder="np. P&ID, CAD 3D, Dokumentacja techniczna"
              />
            </div>

            <div className="mb-4">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Zdjęcie główne</label>
              <input type="file" accept="image/*" ref={coverRef} onChange={handleCoverChange} className="text-sm text-slate-500" />
              {coverData && <img src={coverData} alt="" className="mt-2 w-full aspect-video object-cover rounded border border-slate-200" />}
            </div>
            ...
            <div className="flex gap-3">
              <button type="submit" disabled={saving}
                className="flex-1 px-4 py-2.5 bg-brand text-white font-bold rounded text-sm hover:opacity-90 transition-opacity disabled:opacity-60"
              >
                {saving ? 'Zapisywanie…' : isEditing ? 'Zapisz' : 'Dodaj usługę'}
              </button>
              {isEditing && (
                <button type="button" onClick={reset}
                  className="px-4 py-2.5 border border-slate-200 rounded text-sm font-semibold text-slate-600 hover:bg-slate-50"
        ...
        {isEditing && tab === 'gallery' && (
          <form onSubmit={saveGallery}>
            <h3 className="text-base font-bold text-slate-800 mb-4">Zdjęcia galerii</h3>
            <input type="file" accept="image/*" multiple ref={galleryRef} onChange={handleGalleryChange} className="text-sm text-slate-500" />
            {galleryData.length > 0 && (
            ...
            <p className="text-xs text-slate-400 mt-2 mb-4">Wybierz kilka zdjęć – zastąpią obecną galerię.</p>
            <button type="submit" disabled={saving}
              className="w-full px-4 py-2.5 bg-brand text-white font-bold rounded text-sm hover:opacity-90 transition-opacity disabled:opacity-60"
            >
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <h3 className="text-base font-bold text-slate-800 mb-4">Lista usług</h3>
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-4">
            <img src="/Projekt bez nazwy-4.png" alt="Wenta" className="w-24 animate-pulse opacity-60" />
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest">Ładowanie…</p>
          </div>
        ) : !services.length && <p className="text-sm text-slate-400">Brak usług.</p>}

        <div className="flex flex-col gap-2">
          {services.map(s => (
            <div key={s.id} className="flex items-center gap-3 p-3 border border-slate-100 rounded-lg">
              {s.img
                ? <img src={s.img} alt="" className="w-14 h-10 object-cover rounded flex-shrink-0" />
                : <div className="w-14 h-10 bg-slate-100 rounded flex-shrink-0" />
              }
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm text-slate-800 truncate">{s.title}</div>
                <div className="text-xs text-slate-400 truncate">{s.short_desc || '—'}</div>
              </div>
